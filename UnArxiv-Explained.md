# "UnArxiv" — Simplifying Research for Undergraduates

## Executive Summary

You're about to build **"UnArxiv"** — a locally-run AI system that transforms dense, jargon-heavy research paper abstracts into clear, understandable summaries for first-year computer science students. Think of it as a "translator" that keeps all the meaning but removes the academic gatekeeping.
...
> "We propose a novel approach leveraging structured sparsity and low-rank approximations to reduce computational complexity in attention mechanisms, achieving 2.3× speedup with <1% accuracy degradation on the GLUE benchmark."

Your brain: 🤔 *"What is structured sparsity? What's a low-rank approximation? Why should I care about GLUE? Is this relevant to my interests?"*

You close the tab. Back to scrolling.

**This is the problem.** Research papers use compressed, technical language designed for experts. If you're not yet an expert, you're excluded—not because you're not smart enough, but because you haven't learned the dialect.

### Why Solve It This Way?

You could:
- Use ChatGPT or Claude → But then your data goes to OpenAI's servers. Privacy? Gone.
- Bookmark every paper and read it all → Time-consuming. Unsustainable.
- Take a semester-long course on research papers → Too slow. You want to stay current *now*.

**What you're building instead:** A **private, local AI** that runs on your machine, offline, free. You own it. You control it. And it's fine-tuned specifically on the "abstraction task"—making complex ideas simple.
...
|-------|------------------|------------------|-----------|--------|
| Llama 3.1 8B | ~30GB | ~8-10GB | ❌ Unstable, OOM crashes |
| Mistral 7B | ~26GB | ~7-9GB | ⚠️ Risky, slow due to memory swaps |
| **Qwen 2.5 3B** | **~11GB** | **~4-5GB** | ✅ Safe, stable, room for gradients |
| Phi-3.5 Mini | ~14GB | ~5-6GB | ✅ Also viable, slightly slower than Qwen |
...
2. You send them to a **"Teacher" model** — Google's Gemini 1.5 Flash or OpenAI's GPT-4o mini (the expensive smart model).
3. The teacher rewrites each abstract in **"high school English,"** explaining concepts, avoiding jargon.
4. You collect these pairs: `{Hard Abstract → Simple Summary}`.
5. Your Qwen-2.5-3B (the "student") learns from these examples.

**Why this works:** The student doesn't just learn "summarization"—it learns the *specific style* of simplification you want. It learns that "transformer attention mechanism" becomes "the way AI models focus on different parts of input" or "a technique that lets AI decide what parts of information are most important."
...
- **XPU device** — Intel's abstraction layer for GPU operations.
...
```
GPU VRAM Allocation:
├─ Qwen-2.5-3B (4-bit): ~4.5GB
├─ LoRA adapters: ~0.3GB
├─ Gradients & optimizer states: ~2GB
├─ Activations (batch size 1, 512 tokens): ~0.8GB
└─ Buffer (for safety): ~0.4GB
Total: ~8GB ✅ (Fits perfectly)
```
...
3. **Hit the Gemini API** (or Claude, or GPT-4o mini):
   - Gemini 1.5 Flash: Free tier allows ~15M tokens/month. 1,500 abstracts × 200 words ≈ 300k tokens. **You're well within free tier.**
   - Cost: $0 if you use free tier properly. 💰
...
gradient_accumulation_steps = 4
    # Simulates batch_size = 4 without needing 4× VRAM
...
max_seq_length = 512
    ...
    # 1 Arxiv abstract typically: 150 words ≈ 200 tokens
    # 1 Simplified summary: 150 words ≈ 200 tokens
...
num_train_epochs = 3
    ...
    #   Effective updates = 1,000 / 1 × 4 × 3 = 12,000 steps
...
Step 1: Load Qwen-2.5-3B (Quantized 4-bit)
  └─> Model is loaded from HuggingFace Hub
  └─> Immediately converted to 4-bit using IPEX-LLM's quantizer
  └─> Moved to XPU device (your Arc GPU)

Step 2: Wrap with LoRA
  └─> Adapter matrices are initialized (small, random)
  └─> Only adapters will be updated during training
  └─> Main model stays frozen

Step 3: Load & Tokenize Dataset
  └─> Read JSONL file with {instruction, input, output}
  └─> Combine into single text: "Simplify this abstract:\n[input]\n\n[output]"
...
Step 4: Training Loop
  For 3 epochs:
    For each batch of 1 example:
      └─> Forward pass: Run Qwen on the tokens
      └─> Compute loss: How wrong is the prediction?
      └─> Backward pass: Calculate gradients
      └─> Accumulate gradients (4 steps before update)
      └─> Update only LoRA weights
      └─> Log metrics (loss, learning rate, time)

Step 5: Save Adapters
  └─> Save LoRA weights (~15MB)
  └─> Save config (LoRA rank, target modules, etc.)

Step 6: Inference (Using Your Model)
  └─> Load Qwen-2.5-3B
  └─> Load LoRA adapters
  └─> Merge adapters into the model (or use separately)
  └─> Run on new abstracts:
...
4. **Customization**: With an API, you're stuck with OpenAI's "house style" of simplification. Fine-tune your own model → you control the style completely.
...
4. **Model overfits (works on training data, fails on new abstracts):**
   - *Prevention*: You have 3 epochs × 1,000 examples = only 3,000 gradient updates. Overfitting is unlikely unless your data is homogeneous.
...
In a world where AI access is becoming more equal, expertise in **localized, fine-tuned, private AI** is underrated. You're not just building a toy. You're building infrastructure for a future where intelligence is local, personalized, and owned by users—not corporations.
...
**Estimated Project Complexity:** ⭐⭐⭐⭐ (Advanced, but structured)  
